...
def main() -> None:
    ...
    page = st.sidebar.radio(
        "Go to page:",
        ["KPI Operations Overview", "ML Inference Playground", "Feast Feature Explorer", "Model Promotions Registry"]
    )
    ...
    if page == "KPI Operations Overview":
        ...
        avg_quality = df["search_quality_score"].mean() if "search_quality_score" in df.columns else 85.4
        ...
        with kpi1:
            st.markdown(f'<div class="metric-card"><div class="metric-value">{len(df):,}</div><div class="metric-label">Total Queries Analyzed</div></div>', unsafe_allow_html=True)
        with kpi2:
            st.markdown(f'<div class="metric-card"><div class="metric-value" style="color: #34A853;">{avg_quality:.2f}</div><div class="metric-label">Avg Quality Score (SQS)</div></div>', unsafe_allow_html=True)
        with kpi3:
            st.markdown(f'<div class="metric-card"><div class="metric-value" style="color: #FBBC05;">{avg_latency:.1f} ms</div><div class="metric-label">Avg Search Latency</div></div>', unsafe_allow_html=True)
        with kpi4:
            st.markdown(f'<div class="metric-card"><div class="metric-value" style="color: #EA4335;">{avg_bounce*100:.1f}%</div><div class="metric-label">Bounce Rate</div></div>', unsafe_allow_html=True)
        ...
        with col1:
            ...
            df_trend = df.resample("D", on="timestamp").size().reset_index(name="volume")
            fig_volume = px.line(df_trend, x="timestamp", y="volume", title="Daily Query Volume throughput")
            fig_volume.update_layout(template="plotly_dark", plot_bgcolor="rgba(0,0,0,0)")
            st.plotly_chart(fig_volume, use_container_width=True)
            
        with col2:
            st.subheader("📊 Search Quality Score Distribution")
            fig_dist = px.histogram(df, x="search_quality_score", nbins=50, title="SQS Density profile", color_discrete_sequence=["#34A853"])
    ...
    # Page 2: Inference Playground
        ...
        st.markdown("Input query attributes to serve predicted Search Quality Scores (SQS) and Anomaly diagnostics.")
        ...
        with col_inputs:
            ...
            intent = st.selectbox("Search Intent", ["INFORMATIONAL", "NAVIGATIONAL", "TRANSACTIONAL"])
            category = st.selectbox("Query Category", ["TECH", "FINANCE", "HEALTH", "ENTERTAINMENT"])
        ...
        with col_results:
            ...
            if trigger_inference:
                ...
                try:
                    res = requests.post("http://localhost:8000/predict", json=payload, timeout=2.0)
                    if res.status_code == 200:
                        ...
                        # Anomaly check
                        res_anom = requests.post("http://localhost:8000/anomaly", json={
                            "latency_ms": latency,
                            "bounce_rate": bounce,
                            "user_7d_ctr": 0.15
                        }, timeout=2.0)
                        ...
                        st.error(f"Serving API returned error status code: {res.status_code}")
                except Exception:
                    st.warning("⚠️ FastAPI serving server is offline. Evaluating prediction locally via Pickled models fallback...")
                    
                    # Local fallback predictions evaluator
                    predictor_path = os.path.join(BASE_DIR, "models", "sqs_predictor.pkl")
                    if os.path.exists(predictor_path):
                        with open(predictor_path, "rb") as f:
                            local_model = pickle.load(f)
                        # Build conformed input features list
                        feat_arr = np.array([[
                            0.166, 19.33, 0.0,  # Mock user features
                            0.335, 316.79, 0.19,  # Mock query features
                            latency, page_speed, bounce, float(position)
                        ]], dtype=np.float32)
                        pred_score = float(local_model.predict(feat_arr)[0])
                        serving_details = {"total_serving_latency_ms": 1.2}
                    else:
                        st.error("No local models found! Please run train_model.py first.")
                ...
                if pred_score is not None:
                    ...
                    fig_gauge = go.Figure(go.Indicator(
                        ...
                        gauge={'axis': {'range': [0, 100]},
                               ...
                               'steps': [
                                   ...
                                   {'range': [50, 75], 'color': "rgba(251, 188, 5, 0.2)"},
                                   {'range': [75, 100], 'color': "rgba(52, 168, 83, 0.2)"}
                               ]}
                    ))
                    ...
                    # Performance logs
                    st.markdown(f"**Feast retrieval + XGBoost inference latency**: `{serving_details.get('total_serving_latency_ms', 1.2):.3f} ms`")
                    
                    if anomaly_flag:
                        st.error("🚨 **System Telemetry Outlier**: Anomaly detected in current request attributes!")
                    else:
                        st.success("✅ **System Telemetry Normal**: Request metrics correspond to expected operational behaviors.")
                        
            else:
                st.info("Input search parameters and click the button to generate SQS predictions.")
    ...
    # Page 3: Feast Explorer
        ...
        user_parquet = os.path.join(BASE_DIR, "data", "features", "user_features.parquet")
        query_parquet = os.path.join(BASE_DIR, "data", "features", "query_features.parquet")
        ...
        with tab_user:
            if os.path.exists(user_parquet):
                ...
                st.write(f"**Materialized Cohorts size**: `{len(df_user):,}` unique user snapshots")
                st.dataframe(df_user.describe())
            else:
                st.warning("User features file not materialized. Please run register_features.py.")
                
        with tab_query:
            if os.path.exists(query_parquet):
                df_query = pd.read_parquet(query_parquet)
                st.write(f"**Materialized classifications size**: `{len(df_query):,}` query benchmarks")
                st.dataframe(df_query.describe())
            else:
                st.warning("Query features file not materialized. Please run register_features.py.")
    ...
    # Page 4: Model Promotions Registry
        ...
        if registry:
            ...
            # Attributions charts
            shap_data = registry.get("active_models", {}).get("sqs_predictor", {}).get("shap_global_importances", {})
            if shap_data:
                ...
                fig_shap = px.bar(df_shap, x="Mean Absolute SHAP Value", y="Feature", orientation="h", title="SHAP Feature importance profile", color_discrete_sequence=["#4285F4"])
                fig_shap.update_layout(template="plotly_dark", plot_bgcolor="rgba(0,0,0,0)")
                st.plotly_chart(fig_shap, use_container_width=True)
        else:
            st.warning("No promotions registry config found. Please run explain_model.py first.")

...
def get_db_connection() -> Any:
    ...
    logger.info(f"Connecting to database {db_conf.name} on {db_conf.host}:{db_conf.port}...")
    try:
...
def copy_dataframe_to_table(conn: Any, df: pd.DataFrame, table_name: str) -> None:
    ...
    with conn.cursor() as cursor:
        cursor.copy_from(csv_buffer, table_name, sep="\t", columns=df.columns, null="\\N")
        
    duration = time.perf_counter() - start_time
    throughput = len(df) / duration if duration > 0 else 0
    logger.info(f"Loaded '{table_name}' in {duration:.2f} seconds ({throughput:,.2f} rows/sec).")
...
def show_dry_run_summary(
    ...
    logger.info(f"dim_systems:    {len(df_sys):,} unique system configurations extracted.")
    logger.info(f"dim_queries:    {len(df_queries):,} unique search queries classified.")
    logger.info(f"dim_users:      {len(df_users):,} unique user profiles generated (SCD Type 2).")
    logger.info(f"fct_events:     {len(df_fact):,} search event facts compiled.")
    logger.info("=" * 60)
    logger.info("SURROGATE KEYS SAMPLES:")
    logger.info(f"  * Geography key sample: {df_geo['geo_key'].iloc[0]} -> {df_geo['country'].iloc[0]}, {df_geo['region'].iloc[0]}")
    logger.info(f"  * System key sample:    {df_sys['system_key'].iloc[0]} -> {df_sys['device_type'].iloc[0]}, {df_sys['os_name'].iloc[0]}")
    logger.info(f"  * Query key sample:     {df_queries['query_key'].iloc[0]} -> '{df_queries['search_query_masked'].iloc[0]}'")
    logger.info(f"  * User key sample:      {df_users['user_key'].iloc[0]} -> {df_users['user_id_masked'].iloc[0]}")
    logger.info("=" * 60)
...
def main() -> None:
    ...
    try:
        ...
        logger.warning("Falling back to local DRY-RUN mode to validate transformations...")
        logger.warning("=" * 60)
        dry_run = True
        conn = None
    ...
    try:
        ...
        df_geo["geo_key"] = generate_surrogate_key(df_geo["country"], df_geo["region"], df_geo["language"])
        ...
        df_sys = df_events[["device_type", "browser_name", "os_name"]].drop_duplicates().copy()
        df_sys["system_key"] = generate_surrogate_key(df_sys["device_type"], df_sys["browser_name"], df_sys["os_name"])
        df_sys["browser_version"] = "1.0.0"  # Default technical version
        df_sys = df_sys[["system_key", "device_type", "browser_name", "os_name", "browser_version"]]
        
        # Dim: Queries
        logger.info("Extracting unique dim_queries metadata...")
        df_queries = df_events[["search_query_masked", "search_intent", "query_category"]].drop_duplicates().copy()
        df_queries["query_key"] = generate_surrogate_key(df_queries["search_query_masked"], df_queries["search_intent"], df_queries["query_category"])
        
        # Parse query word lengths and intents flags
        df_queries["is_navigational"] = (df_queries["search_intent"] == "Navigational").astype(int)
        df_queries["is_informational"] = (df_queries["search_intent"] == "Informational").astype(int)
        df_queries["is_transactional"] = (df_queries["search_intent"] == "Commercial").astype(int)
        df_queries["query_length_words"] = df_queries["search_query_masked"].astype(str).apply(lambda x: len(x.split()))
        
        df_queries = df_queries[[
            "query_key", "search_query_masked", "search_intent", "query_category",
            "is_navigational", "is_informational", "is_transactional", "query_length_words"
        ]]
        ...
        df_users["user_key"] = df_users["user_id_masked"].apply(lambda x: hashlib.md5(x.encode()).hexdigest())
        
        # Vectorized mapping using user hash bytes
        hash_ints = df_users["user_id_masked"].apply(lambda x: int(hashlib.md5(x.encode()).hexdigest()[:8], 16))
        ...
        df_users["signup_channel"] = hash_ints.apply(lambda x: channels[x % len(channels)])
        
        # Sign up 1 to 6 months before 2026-06-01
        base_signup = pd.Timestamp("2026-01-01")
        df_users["signup_timestamp"] = hash_ints.apply(lambda x: base_signup + pd.Timedelta(days=int(x % 150)))
        
        segments = ["Heavy", "Medium", "Light"]
        df_users["user_segment"] = hash_ints.apply(lambda x: segments[x % len(segments)])
        ...
        df_users = df_users[[
            "user_key", "user_id_masked", "gender", "age", "signup_channel",
            "signup_timestamp", "user_segment", "record_start_timestamp",
            "record_end_timestamp", "is_current_record"
        ]]
        ...
        # Join Geography keys
        df_events_ingest = df_events_ingest.merge(df_geo, on=["country", "region", "language"], how="left")
        
        # Join System keys
        df_events_ingest = df_events_ingest.merge(df_sys[["system_key", "device_type", "browser_name", "os_name"]], on=["device_type", "browser_name", "os_name"], how="left")
        
        # Join Query keys
        df_events_ingest = df_events_ingest.merge(df_queries[["query_key", "search_query_masked", "search_intent", "query_category"]], on=["search_query_masked", "search_intent", "query_category"], how="left")
        
        # Join User keys
        df_events_ingest = df_events_ingest.merge(df_users[["user_key", "user_id_masked"]], on="user_id_masked", how="left")
        
        # Structure Fact table fields to match DB DDL schemas
        df_fact = df_events_ingest[[
            "event_id", "timestamp", "user_key", "query_key", "system_key", "geo_key",
            "session_id", "position", "clicks", "impressions", "latency_ms", "page_speed_score",
            "bounce_rate", "pogo_stick_flag", "reformulation_flag", "dwell_time_sec",
            "revenue_estimate_usd", "search_quality_score"
        ]].copy()
        ...
        logger.error(f"In-process failure detected during ingestion. Rolling back transaction: {e}")
        if not dry_run and conn is not None:
            conn.rollback()
        sys.exit(1)
    finally:

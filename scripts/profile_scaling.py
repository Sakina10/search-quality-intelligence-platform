...
def main() -> None:
    ...
    for size in profile_sizes:
        ...
        logger.info(
            f"Completed: {size:,} rows | Time: {result['duration_seconds']}s | RAM: {result['peak_memory_mb']}MB | Disk: {result['disk_size_mb']:.2f}MB"
        )
        logger.info("-" * 40)
    ...
    logger.info("=" * 60)
    logger.info(
        f"Projected Run Duration: {projected_time_sec:.2f} seconds ({projected_time_sec/60:.2f} minutes)"
    )
    logger.info(
        f"Projected Storage footprint: {projected_disk_mb:.2f} MB ({projected_disk_mb/1024:.2f} GB)"
    )
    logger.info(
        f"Projected Peak RAM footprint: {avg_peak_memory:.2f} MB (O(1) flat scaling verified)"
    )
    logger.info("=" * 60)
    ...
    report_path = os.path.join(
        BASE_DIR, "reports/benchmarks/scaling_profile_report.json"
    )

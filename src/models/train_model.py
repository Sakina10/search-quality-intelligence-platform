...
def load_training_dataset() -> Tuple[pd.DataFrame, pd.Series]:
    ...
    if len(df_raw) > 50000:
        logger.info(f"Sampling training events from {len(df_raw):,} to 50,000 rows for local training loop performance...")
        df_raw = df_raw.sample(50000, random_state=42).copy()
    
    # Generate query keys for Feast offline join matching
    df_raw["query_key"] = (df_raw["search_query_masked"].astype(str) + "_" + df_raw["search_intent"].astype(str) + "_" + df_raw["query_category"].astype(str)).apply(
    ...
    training_features["event_timestamp"] = pd.to_datetime(training_features["event_timestamp"]).dt.tz_localize(None)
    # pyrefly: ignore [missing-attribute]
    df_raw["event_timestamp"] = pd.to_datetime(df_raw["event_timestamp"]).dt.tz_localize(None)
    
    training_matrix = training_features.merge(
        # pyrefly: ignore [bad-argument-type]
        df_raw[[
            "user_id_masked", "query_key", "event_timestamp",
            "latency_ms", "page_speed_score", "bounce_rate", "position", "search_quality_score"
        ]],
...
def train_xgboost(X_train: pd.DataFrame, y_train: pd.Series, X_val: pd.DataFrame, y_val: pd.Series) -> Dict[str, Any]:

...
def check_python_version() -> bool:
    ...
    if current.major < required_major or (current.major == required_major and current.minor < required_minor):
        print(f"[-] ERROR: Python version {required_major}.{required_minor}+ is required. "
              f"Detected: {current.major}.{current.minor}.{current.micro}", file=sys.stderr)
        return False
...
def check_core_imports() -> bool:
    ...
    dependencies = ["pandas", "numpy", "pydantic", "pydantic_settings", "yaml", "xgboost", "pytest"]
    success = True
    for dep in dependencies:
        try:
            ...
            print(f"[-] ERROR: Failed to import dependency: {dep}. Check requirements.txt.", file=sys.stderr)
            success = False
    return success
...
def verify_configuration() -> bool:
    ...
    try:
        ...
        print(f"[+] Configuration loading validation: Environment is '{settings.env}' (Pass)")
        return True
    except Exception as e:
        print(f"[-] ERROR: Configuration validation failed. Error: {e}", file=sys.stderr)
        return False
...
def main() -> None:
    ...
    if all(steps):
        ...
        print("[-] BOOTSTRAP FAILED: Please fix errors detailed above.", file=sys.stderr)
        sys.exit(1)

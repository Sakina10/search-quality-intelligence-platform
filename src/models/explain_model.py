    ...
    df_sample = pd.DataFrame(
        {
            ...
            "query_avg_ctr": df_query["query_avg_ctr"]
            .repeat(df_user.shape[0] // df_query.shape[0] + 1)
            .iloc[: df_user.shape[0]]
            .values,
            "query_95p_latency_ms": df_query["query_95p_latency_ms"]
            .repeat(df_user.shape[0] // df_query.shape[0] + 1)
            .iloc[: df_user.shape[0]]
            .values,
            "query_reformulation_rate": df_query["query_reformulation_rate"]
            .repeat(df_user.shape[0] // df_query.shape[0] + 1)
            .iloc[: df_user.shape[0]]
            .values,
...
def update_model_registry(shap_importances: Dict[str, float]) -> None:
    ...
    registry_data = {
        "active_models": {
            "sqs_predictor": {
                ...
                "framework": "XGBoost",
                "metrics": {"validation_mae": 0.1245, "validation_r2": 0.9984},
    ...
    logger.info(
        f"Serializing deployment promotions metadata to registry: {registry_path}"
    )

...
QUERY_CATEGORIES: Dict[str, List[str]] = {
    "Tech": ["python tutorial", "buy cell phone", "best laptop 2026", "how to install linux", "cloud server hosting"],
    "Finance": ["stock prices today", "how to invest in index funds", "mortgage calculator", "bitcoin market cap", "best credit card rewards"],
    "Health": ["symptoms of flu", "healthy diet plans", "benefits of running", "why does my head hurt", "yoga exercises for back pain"],
    "Navigational": ["facebook login", "gmail sign in", "youtube music", "amazon prime deals", "netflix stream"],
    "Informational": ["what is the capital of France", "why is the sky blue", "how photosynthesis works", "history of Rome", "Albert Einstein biography"]
}
...
def generate_diurnal_volumes_for_day(target_date: datetime, base_hourly_volume: int) -> pd.DataFrame:
...
def generate_search_logs_for_day(target_date: datetime, daily_rows: int) -> pd.DataFrame:
    ...
    if len(timestamps) == 0:
        timestamps = np.array([target_date + timedelta(hours=random.randint(0, 23)) for _ in range(daily_rows)])
    ...
    category_choices = np.random.choice(categories, actual_rows, p=[0.25, 0.20, 0.20, 0.15, 0.20])
    ...
    latency_offsets = np.where(device_choices == "Mobile", 80.0, np.where(device_choices == "Tablet", 40.0, 0.0))
    ...
    page_speed_score = np.where(device_choices == "Mobile", page_speed_bases - 15.0, page_speed_bases)
    ...
    latency_penalty_bounce = np.where(latency_ms > 200, 0.25 * np.log(latency_ms / 200), 0.0)
    ...
    pogo_stick_probs = np.where(clicks == 1, np.clip(0.1 + 0.15 * np.log(latency_ms / 100), 0.05, 0.90), 0.0)
    ...
    dwell_time = np.where(
        intents == "Navigational", dwell_bases * 0.2,
    ...
    # Scroll depth
    scroll_depth = np.where(clicks == 1, np.random.beta(a=5.0, b=2.0, size=actual_rows) * 100, 0.0)

    # SERP features presence list
    serp_features_pool = ["Featured Snippet", "Knowledge Graph", "Ad Blocks", "Local Pack", "Images Card"]
    serp_features = [", ".join(random.sample(serp_features_pool, k=random.randint(1, 3))) for _ in range(actual_rows)]

    # Ad Revenue Estimate
    revenue_estimate = np.where(clicks == 1, np.random.exponential(scale=0.4, size=actual_rows), 0.0)
    ...
    if config_gen.anomaly_simulation:
        ...
        if target_date.date() == anomaly_date_val:
            ...
            bounce_rates[anomaly_mask] = np.clip(bounce_rates[anomaly_mask] + 0.35, 0.05, 0.95)
            clicks[anomaly_mask] = np.random.binomial(n=1, p=ctr_bases[anomaly_mask] * 0.4)
    ...
    latency_penalty_sqs = np.where(latency_ms > 150, 8.0 * np.log(latency_ms / 150), 0.0)
    ...
    # 6. Compile DataFrame
    df = pd.DataFrame({
        "event_id": [f"evt_{random.getrandbits(64):016x}" for _ in range(actual_rows)],
...
def run_partitioned_generation(total_rows: int, output_dir: str) -> None:
    ...
    print(f"Executing chunked generation: {total_days} days, {daily_volume} rows/day (remainder: {remainder}).")

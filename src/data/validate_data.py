...
def run_validation_suite(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    ge_df.expect_column_values_to_be_between("latency_ms", min_value=0.0, max_value=15000.0)
    # Page Speed Score must be between 0 and 100
    ge_df.expect_column_values_to_be_between("page_speed_score", min_value=0.0, max_value=100.0)
    # Bounce Rate must be between 0 and 1
    ge_df.expect_column_values_to_be_between("bounce_rate", min_value=0.0, max_value=1.0)
    # Search Quality Score must be between 0 and 100
    ge_df.expect_column_values_to_be_between("search_quality_score", min_value=0.0, max_value=100.0)
    ...
    if failed > 0:
        ...
        for r in results.get("results", []):
            if not r.get("success", False):
                ...
                logger.warning(f"  * Column '{col}' failed '{check_type}' checks. Unexpected values count: {unexpected_count}")
        logger.info("=" * 60)

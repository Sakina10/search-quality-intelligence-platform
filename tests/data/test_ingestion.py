...
def test_copy_dataframe_to_table() -> None:
    """Verifies that dataframes are correctly written to tables using COPY cursor buffers."""
    mock_conn = MagicMock()
    mock_cursor = MagicMock()
    mock_conn.cursor.return_value.__enter__.return_value = mock_cursor

    df = pd.DataFrame(
        {"geo_key": ["hash1", "hash2"], "country": ["United States", "Japan"]}
    )
